...
let _win2panels = {}; // browserWindow to panelIDs
...
Panel.findWindow = function ( panelID ) {
  for ( let win in _win2panels ) {
    let panelIDs = _win2panels[win];
...
ipcMain.on('panel:add', (event, panelID) => {
  ...
  let panelIDs = _win2panels[browserWin];
  if ( panelIDs === undefined ) {
    panelIDs = [];
    _win2panels[browserWin] = panelIDs;

    // make sure we clear out the panels when window closed
    browserWin.once('closed', () => {
      delete _win2panels[browserWin];
    });
  }
